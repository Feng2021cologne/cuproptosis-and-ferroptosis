...
geneSets=read.table("GeneList.txt",header = T,sep = "\t") 
geneSets=geneSets[,c(1,ncol(geneSets))]
geneSets0 = list()
for (i in unique(geneSets$Category)) {
  geneSets0[[i]] = geneSets$Symbol[geneSets$Category==i]
}

ssgseaScore=gsva(data, geneSets0, method='ssgsea', kcdf='Gaussian', abs.ranking=TRUE)
...
p+stat_compare_means(aes(group=`Cup-Ferroptosis cluster`),
